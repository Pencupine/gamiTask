...
gulp.task('build-css', function() {
	return gulp.src('src/**/*.css').pipe(css()).pipe(gulp.dest('app/'));
});

gulp.task('build-scss', function() {
	return gulp.src('src/**/*.scss').pipe(scss()).pipe(css()).pipe(gulp.dest('app/'));
});
...
gulp.task(
	...
	gulp.series('copy', 'build', () => {
		...
		return new Promise(function(resolve, reject) {
			console.log('Finished copying and building. Now starting your Electron App.');
			resolve();
		});
	})
);
